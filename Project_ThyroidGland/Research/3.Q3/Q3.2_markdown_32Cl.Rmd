...
The task assigned was to study the RNA expression in a set of samples First, let's set the parameters used throughout the scripts.

```{r}
  p_value = .05 # p-value threshold for statistic significance
  minSize = 5   # minimum size to keep the pathway
```
...
-   The number of significant down-regulated and up-regulated pathways associated with each morphological cluster
-   The 10 most significant up-regulated pathways
...
# filtering step, to remove potential NAs: 
...
### Step 2: Replace names (to matches the genes symbols used in reactome)
...
cat("-------------------------------\n",
  ...
  geom_col() +
  coord_flip() +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0.05, limits = c(0, 0.2), name = "padj") +
  labs(x = "Pathway", y = "Normalized Enrichment Score",
       title = paste0("Hallmark pathways NES from GSEA \n (", var,")")) +
  theme_minimal()
